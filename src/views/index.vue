<template>
  <home-layer>
    <template slot="header">
      ...
      <i class="iconfont icon-username username" @click.stop="$router.push({name: 'user'})"></i>
    </template>
    <template slot="nav">
      <ul class="flex-horizontal-nowrap flex-horizontal-around nav">
        <router-link tag="li" :to="{name: 'recommend'}">
          <span>推荐</span>
        </router-link>
        ...
        <router-link tag="li" :to="{name: 'search'}">
          <span>搜索</span>
        </router-link>
      </ul>
    </template>
...
import Player from '@/components/services/player/player.vue'
export default {
  name: 'Index',
  components: {
    HomeLayer,
